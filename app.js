(() => {
  ...
  function initFlashcards() {
    ...
    grid.querySelectorAll(".flashcard").forEach(card => {
      const setFlipped = flipped => {
        ...
        card.setAttribute("aria-label", flipped ? "Hide flashcard answer" : "Reveal flashcard answer");
      };
  ...
  function initQuiz() {
    ...
    let score = 0;
    let answered = false;
    const best = Number(storage.get("hf-studio-best-score") || 0);
    ...
    const renderResult = () => {
      ...
      const previousBest = Number(storage.get("hf-studio-best-score") || 0);
      if (score > previousBest) storage.set("hf-studio-best-score", String(score));
      root.innerHTML = `
  ...
  function getVisited() {
    try { return new Set(JSON.parse(storage.get("hf-studio-visited") || "[]")); }
    catch { return new Set(); }
  }

  function saveVisited(visited) {
    storage.set("hf-studio-visited", JSON.stringify([...visited]));
  }
  ...
  speechRate.addEventListener("change", () => {
    storage.set("hf-studio-speech-rate", speechRate.value);
  ...
  document.getElementById("reset-progress").addEventListener("click", () => {
    storage.remove("hf-studio-visited");
  ...
  const savedRate = storage.get("hf-studio-speech-rate");
